<template>
  <div class="comment">
    <h3>留言反馈</h3>
    <textarea placeholder="请输入你要留下的内容（最多120个字）" 
    maxlength="120" v-model="commentConnent"></textarea>
    <mt-button type="primary" size="large" @click="sendComment">发表留言</mt-button>
    <div class="comment-list">
      <div class="comment-item" v-for="(item, index) in commentList" :key="index">
        <div class="comment-user-and-date">
            第{{index+1}}楼&nbsp;&nbsp;用户：{{item.user_name}}&nbsp;&nbsp;
            留言时间：{{item.add_time | dateFormat}}</div>
        <div class="comment-content" v-text="item.content=='undefined'?'该用户未留言内容':item.content"></div>
      </div>
    </div>
